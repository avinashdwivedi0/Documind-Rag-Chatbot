...
def run():
    ...
    baseline_run = es.run_evaluation_dataset(workspace_id, vectorstore, chain, memory, cases=cases, top_k=3)
    print(f"Baseline run created: {baseline_run['id']}")

    print('Running comparison evaluation...')
    comparison_run = es.run_evaluation_dataset(workspace_id, vectorstore, chain, memory, cases=cases, top_k=3)
    print(f"Comparison run created: {comparison_run['id']}")

    thresholds = {'hit_drop': 0.2, 'mrr_drop': 0.2, 'latency_increase': 1.0}
    result = es.compare_runs(workspace_id, baseline_run['id'], comparison_run['id'], thresholds=thresholds)
