...
class LocalStorageTests(unittest.TestCase):
    def test_turn_is_scoped_to_its_conversation(self):
        with patch.object(chat_history, "load_history", return_value=[]) as load, patch.object(
    ...
    def test_workspace_metadata_and_conversations(self):
        ...
        with patch.object(workspace_store, "get_workspace", return_value=None), patch.object(
            workspace_store, "_write"
        ):
        ...
        with patch.object(workspace_store, "get_workspace", return_value=workspace), patch.object(
            workspace_store, "_write"
        ):
    ...
    def test_document_intelligence_and_graph(self):
        intelligence = extract_intelligence(
            "Acme Corporation acquired River Labs in London.", "deal.pdf"
        )
