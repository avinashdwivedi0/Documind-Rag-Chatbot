...
def extract_intelligence(text: str, filename: str) -> dict:
    """Extract a compact summary of document topics and prominent named entities."""
    words = re.findall(r"[A-Za-z]{4,}", text.lower())
    topics = [
        word for word, _ in Counter(word for word in words if word not in STOP_WORDS).most_common(8)
    ]
    entities = []
...
def build_knowledge_graph(documents: List[Dict]) -> str:
    ...
    for document in documents:
        ...
        lines.append(
            f'{document_id} [label="{safe_name}", shape=box, '
            'style="rounded,filled", fillcolor="#263451", '
            'fontcolor="#f7f9ff"];'
        )
        for entity in document.get("intelligence", {}).get("entities", [])[:6]:
            ...
            lines.append(
                f'{entity_id} [label="{safe_entity}", shape=ellipse, '
                'style=filled, fillcolor="#7258ff", fontcolor="#ffffff"];'
            )
            lines.append(
                f'{document_id} -> {entity_id} [color="#8da0c7", '
                'label="mentions", fontcolor="#8da0c7"];'
            )
    lines.append("}")
    return "\n".join(lines)
