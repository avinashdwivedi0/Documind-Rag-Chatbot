...
class BaseEvaluator:
    ...
    def evaluate_citations(self, expected_sources: Optional[list], retrieved_sources: Optional[list]) -> Dict:
        raise NotImplementedError()
...
class RuleBasedEvaluator(BaseEvaluator):
    ...
    def evaluate_answer(self, expected: str, generated: str) -> Dict:
        ...
        if not exp_tokens:
            return {"score": 0.0, "label": "Not available", "reason": "Expected answer had no tokens"}
    ...
    def evaluate_citations(self, expected_sources: Optional[list], retrieved_sources: Optional[list]) -> Dict:
        ...
        if not expected_sources:
            return {"filename_matches": 0, "filename_total": 0, "page_matches": 0, "page_total": 0, "excerpt_match_rate": "Not available", "overall_match_rate": "Not available"}
        ...
        def _norm_expected(es):
            ...
            for e in es:
                if isinstance(e, dict):
                    out.append({
                        'source': os.path.basename(e.get('source', '')) if e.get('source') else '',
        ...
        def _norm_retrieved(rs):
            ...
            for r in (rs or []):
                if isinstance(r, dict):
                    out.append({'source': os.path.basename(r.get('source', '')) if r.get('source') else '', 'page': r.get('page'), 'excerpt': r.get('excerpt', '')})
                else:
        ...
        for e in expected:
            ...
            if pages:
                ...
                for r in matched:
                    try:
                        if r.get('page') is not None and (int(r.get('page')) + 1) in [int(p) for p in pages]:
                            found = True
                            break
                    except Exception:
                        continue
        ...
        filename_rate = round(filename_matches / filename_total, 3) if filename_total else 'Not available'
        page_rate = round(page_matches / page_total, 3) if page_total else 'Not available'
        excerpt_rate = round(excerpt_matches / excerpt_checks, 3) if excerpt_checks else 'Not available'
...
class LLMEvaluator(BaseEvaluator):
    ...
    def evaluate_answer(self, expected: str, generated: str) -> Dict:
        if not self.enabled:
            return {"score": 0.0, "label": "Unavailable", "reason": "LLM evaluator not configured (missing GROQ_API_KEY)"}
        ...
        try:
            ...
            payload = {"model": "llama-3.1-4b-instant", "messages": [{"role": "user", "content": prompt}], "max_tokens": 200, "temperature": 0}
            ...
            return {"score": 0.0, "label": "Unavailable", "reason": "LLM evaluation failed or returned unparsable output"}
